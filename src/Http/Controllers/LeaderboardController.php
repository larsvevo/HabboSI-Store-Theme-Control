...
class LeaderboardController extends Controller
{
    ...
    public function __invoke(): View
    {
        ...
        $smaragd = User::with(['currencies' => fn ($query) => $query->where('type', 103)])
            ->where('rank', '<', $settings->get('min_staff_rank'))
            ->whereHas('currencies', fn ($query) => $query->where('type', 103))
            ->join('users_currency', 'users.id', '=', 'users_currency.user_id')
            ->where('users_currency.type', 103)
            ->orderBy('users_currency.amount', 'desc')
            ->select('users.*')
            ->limit(5)
            ->get();

        return view('leaderboards', compact('credits', 'duckets', 'diamonds', 'onlineTimes', 'respects', 'achievements', 'smaragd'));
    }
}
